...
class HfReasoning():
    ...
    def __call__(self, prompt: str, image=None, max_tokens=1024, top_p=1,top_k=1, temperature=0.0, **kwargs) -> tuple[str, dict]:
        ...
        # Add "<think>" after the generation prompt
        think_token = self.processor.tokenizer.encode("<think>", add_special_tokens=False)
        inputs["input_ids"] = torch.cat([inputs["input_ids"], torch.tensor([think_token]).to(inputs["input_ids"].device)], dim=-1)
        inputs["attention_mask"] = torch.cat([inputs["attention_mask"], torch.ones((inputs["attention_mask"].shape[0], len(think_token))).to(inputs["attention_mask"].device)], dim=-1)
